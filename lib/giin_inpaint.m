function [ G, pixels, Pstructure, Pinformation ] = giin_inpaint( G, pixels, patches, gparam, liveplot, searchRegion )
...
Patchesknowns = find(unknowns==0);
knowns = searchRegion;
if sum(unknowns<0)+length(Patchesknowns) ~= G.N
    error('Missing vertices !');
end

member = [];
for number = 1:size(knowns)
    if ismember(knowns(number),Patchesknowns)
         member = cat (1, member, knowns(number) );
    end
end
% Structure priority.
...
while ~isempty(currents) || first
    ...
    G = giin_connect(G, news, member, patches, gparam);
